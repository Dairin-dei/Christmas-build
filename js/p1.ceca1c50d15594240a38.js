...
const selfestimation_1 = __importDefault(__webpack_require__(768));
...
class DecorationCard {
    ...
    createDecorationCard(num, isCountNeeded = false) {
        ...
        if (isCountNeeded && this.path && this.count) {
            ...
            domCardCountWrapper.classList.remove(CLASSES.HIDDEN);
...
const definitions_1 = __webpack_require__(125);
const generalFunctions_1 = __webpack_require__(987);
...
const dragAndDrop_1 = __webpack_require__(226);
const CLASSES = {
    ON_OFF_LIGHTS_ACTIVE: "on-off-lights-active",
    ON_OFF_LIGHTS_OFF: "on-off-lights-off",
    RESET_PRESSED: "reset-pressed",
    BUTTON_SNOW_ACTIVE: "button-snow-active",
    ACTIVE: "active",
};
class DecorationParameters {
    constructor() {
        ...
        this.domTreeWrapper = document.querySelector(".trees-wrapper");
        this.domBackgroundWrapper = document.querySelector(".backgrounds-wrapper");
        this.domOrnamentsWrapper = document.querySelector(".ornaments-wrapper");
        this.buttonToggle = document.querySelector(".on-off-lights");
        this.buttonSnow = document.querySelector(".button-snow");
    }
    showParameters() {
        this.showTrees();
        ...
        (0, dragAndDrop_1.activateDragAndDrop)(this.tree);
        this.setEventListenerOnButtonToggleLights();
        this.setEventListenerOnButtonSnow();
        this.addEventListenerOrResetLocalStorage();
    }
    showTrees() {
        this.trees = [];
        ...
        this.placeCards(this.domTreeWrapper, this.trees, false);
    ...
    showBackgrounds() {
        ...
        this.placeCards(this.domBackgroundWrapper, this.backgrounds, false);
    ...
    showChosenOrnaments() {
        this.combineArrayOfOrnaments();
        this.createOrnamentCards();
        this.domOrnamentsWrapper.innerHTML = "";
        this.placeCards(this.domOrnamentsWrapper, this.ornaments, true);
        (0, dragAndDrop_1.dragAndDropsetEventListenerOnOrnaments)(this.tree);
    }
    combineArrayOfOrnaments() {
        this.ornamentsData = [];
        this.ornaments = [];
    ...
    setEventListenerOnTrees() {
        this.domTreeWrapper.onclick = (event) => {
    ...
    setEventListenerOnBackgrounds() {
        this.domBackgroundWrapper.onclick = (event) => {
            this.tree.setBackground(event.target);
            this.tree.localStorageTree.setSettingsForSaving(this.tree);
        };
    }
    setEventListenerOnLights() {
        const buttonsLights = document.querySelectorAll(".color-button");
        buttonsLights.forEach((item) => (item.onclick = () => {
            const color = (0, generalFunctions_1.checkOnVoid)(item.dataset.color, "white");
            this.tree.addLights(color);
        }));
    }
    setEventListenerOnButtonToggleLights() {
        this.buttonToggle.onclick = () => {
            this.buttonToggle.classList.add(CLASSES.ON_OFF_LIGHTS_ACTIVE);
            setTimeout(() => {
                this.buttonToggle.classList.remove(CLASSES.ON_OFF_LIGHTS_ACTIVE);
            }, definitions_1.ANIMATION_TIME_200);
            if (this.buttonToggle.classList.contains(CLASSES.ON_OFF_LIGHTS_OFF)) {
                this.buttonToggle.textContent = "Выкл";
                this.buttonToggle.classList.remove(CLASSES.ON_OFF_LIGHTS_OFF);
                this.tree.isLightsOn = true;
                this.tree.setLights(this.tree.lights.color);
            }
            else {
                this.buttonToggle.textContent = "Вкл";
                this.buttonToggle.classList.add(CLASSES.ON_OFF_LIGHTS_OFF);
    ...
    setEventListenerOnButtonSnow() {
        this.buttonSnow.onclick = () => this.runSnowflakes();
    }
    addEventListenerOrResetLocalStorage() {
        const domResetSettings = document.querySelector(".settings-reset-tree");
        domResetSettings.addEventListener("click", () => {
            domResetSettings.classList.add(CLASSES.RESET_PRESSED);
            setTimeout(() => {
                domResetSettings.classList.remove(CLASSES.RESET_PRESSED);
            }, definitions_1.ANIMATION_TIME_200);
            this.tree = new Tree_1.default();
            this.tree.localStorageTree.setSettingsForSaving(this.tree);
            this.tree.localStorageTree.setToLocalStorage();
            this.setInitialParameters();
            this.showChosenOrnaments();
        });
    }
    runSnowflakes() {
        this.buttonSnow.classList.add(CLASSES.BUTTON_SNOW_ACTIVE);
        setTimeout(() => {
            this.buttonSnow.classList.remove(CLASSES.BUTTON_SNOW_ACTIVE);
        }, definitions_1.ANIMATION_TIME_200);
        if (this.buttonSnow.classList.contains(CLASSES.ACTIVE)) {
            this.buttonSnow.classList.remove(CLASSES.ACTIVE);
            this.tree.isSnowOn = false;
            if (exports.snowInterval) {
                clearInterval(exports.snowInterval);
            }
        }
        else {
            exports.snowInterval = setInterval(snowflakes_1.default, definitions_1.ANIMATION_TIME_200);
            this.buttonSnow.classList.add(CLASSES.ACTIVE);
            (0, snowflakes_1.default)();
            this.tree.isSnowOn = true;
        }
        this.tree.localStorageTree.setSettingsForSaving(this.tree);
    }
...
class LocalStorageFilters {
    ...
    getItemsFromLocalStorage() {
        var _a;
        const gotFilterShape = (0, generalFunctions_1.checkOnVoid)(localStorage.getItem("dd_filterShape"), "");
        if (gotFilterShape) {
            const tempArray = [];
        ...
        const gotFilterColor = (0, generalFunctions_1.checkOnVoid)(localStorage.getItem("dd_filterColor"), "");
        if (gotFilterColor) {
            const tempArray = [];
        ...
        const gotFilterSize = (0, generalFunctions_1.checkOnVoid)(localStorage.getItem("dd_filterSize"), "");
        if (gotFilterSize) {
            const tempArray = [];
        ...
        const gotFilterFavorite = (0, generalFunctions_1.checkOnVoid)(localStorage.getItem("dd_filterFavorite"), "");
        if (gotFilterFavorite) {
            this.filters.filterFavorite = gotFilterFavorite === "true";
        }
        const gotFilterAmount = (0, generalFunctions_1.checkOnVoid)(localStorage.getItem("dd_filterAmount"), "");
        if (gotFilterAmount) {
            const tempConvert = JSON.parse(gotFilterAmount);
            this.filters.filterAmount = tempConvert;
        }
        const gotFilterYear = (0, generalFunctions_1.checkOnVoid)(localStorage.getItem("dd_filterYear"), "");
        if (gotFilterYear) {
            const tempConvert = JSON.parse(gotFilterYear);
            this.filters.filterYear = tempConvert;
        }
        const gotSortingMethod = (0, generalFunctions_1.checkOnVoid)(localStorage.getItem("dd_sortingMethod"), "");
        if (gotSortingMethod) {
        ...
        const gotIsMusicOn = (0, generalFunctions_1.checkOnVoid)(localStorage.getItem("dd_isMusicOn"), "");
        this.isMusicOn = gotIsMusicOn == "true";
    }
}
...
class LocalStorageToys {
    ...
    getItemsFromLocalStorage() {
        const gotChosenToys = (0, generalFunctions_1.checkOnVoid)(localStorage.getItem("dd_chosenToys"), "");
        if (gotChosenToys) {
...
class LocalStorageTree {
    ...
    setSettingsForSaving(tree) {
        ...
        this.lights = tree.lights.color;
        this.isLightsOn = tree.isLightsOn;
        this.isSnowOn = tree.isSnowOn;
    }
    setToLocalStorage() {
        ...
        localStorage.setItem("dd_isSnowOn", String(this.isSnowOn));
    }
    getItemsFromLocalStorage() {
        const gotOrnaments = (0, generalFunctions_1.checkOnVoid)(localStorage.getItem("dd_treeOrnaments"), "");
        if (gotOrnaments) {
        ...
        const gotTree = (0, generalFunctions_1.checkOnVoid)(localStorage.getItem("dd_tree"), "");
        this.tree = gotTree === "" ? 0 : Number(gotTree);
        const gotBackground = (0, generalFunctions_1.checkOnVoid)(localStorage.getItem("dd_background"), "");
        this.background = gotBackground === "" ? 0 : Number(gotBackground);
        const gotLights = (0, generalFunctions_1.checkOnVoid)(localStorage.getItem("dd_lights"), "");
        this.lights = gotLights;
        const gotIsLightsOn = (0, generalFunctions_1.checkOnVoid)(localStorage.getItem("dd_isLightsOn"), "");
        this.isLightsOn = gotIsLightsOn === "true";
        const gotIsSnowOn = (0, generalFunctions_1.checkOnVoid)(localStorage.getItem("dd_isSnowOn"), "");
        this.isSnowOn = gotIsSnowOn === "true";
    }
}
...
class Ornaments {
    constructor(ornamentsArray) {
        ...
        this.chosenSpan = document.querySelector(".chosen-span");
    ...
    placeAllCards(loadFromStorage) {
        ...
        const cardsArray = this.domCards.querySelectorAll(".card");
        cardsArray.forEach((elem) => {
            this.domCards.removeChild(elem);
        });
        const domFragment = document.createDocumentFragment();
        ...
        this.domCards.appendChild(domFragment);
        const domCardElements = this.domCards.querySelectorAll(".card");
    ...
    setEventListener() {
        this.domCards.onclick = (event) => {
            ...
            if (card) {
                ...
                this.chosenSpan.textContent = String(definitions_1.CHOSEN_TOYS.length);
            }
        };
    }
    changeChoosingOrnament(domElement) {
        domElement.classList.toggle(CLASSES.CARD_ACTIVE);
        const domCardRibbon = domElement.querySelector(".card-ribbon");
        domCardRibbon.classList.toggle(CLASSES.RIBBON_ACTIVE);
    }
...
const CLASSES = {
    FILTERS_ACTIVE_BUTTON: "filters-active-button",
    FILTERS_COLOR_ACTIVE_BUTTON: "filters-color-active-button",
...
class Parameters {
    constructor() {
        ...
        this.domShapeFilterButtons = document.querySelectorAll(".button-shape");
        this.domColorFilterButtons = document.querySelectorAll(".button-color");
        this.domSizeFilterButtons = document.querySelectorAll(".button-size");
        this.domFilterFavorite = document.querySelector(".label-favorite");
        this.domInputFavorite = document.querySelector(".input-favorite");
        this.buttonAudio = document.querySelector(".button-sound");
    }
    ...
    createRangeFilters() {
        rangefilters.createAmountRangeFilter();
        rangefilters.createYearRangeFilter();
    }
    addEventListeners() {
    ...
    addEventListenersOnShapeFilters() {
        this.domShapeFilterButtons.forEach((domFilterButton) => {
            domFilterButton.addEventListener("click", (event) => {
                this.filters.filterShape = handleEvent(event, this.domShapeFilterButtons);
                this.ApplyParameters();
            });
        });
        function handleEvent(event, domShapeFilterButtons) {
            const currentDomElement = event.target;
            currentDomElement.classList.toggle(CLASSES.FILTERS_ACTIVE_BUTTON);
            const tempFilterArray = [];
            domShapeFilterButtons.forEach((button) => {
                var _a;
                if (button.classList.contains(CLASSES.FILTERS_ACTIVE_BUTTON)) {
    ...
    addEventListenersOnColorFilters() {
        this.domColorFilterButtons.forEach((domFilterButton) => {
            domFilterButton.addEventListener("click", (event) => {
                this.filters.filterColor = handleEvent(event, this.domColorFilterButtons);
                this.ApplyParameters();
            });
        });
        function handleEvent(event, domColorFilterButtons) {
            const currentDomElement = event.currentTarget;
            currentDomElement.classList.toggle(CLASSES.FILTERS_COLOR_ACTIVE_BUTTON);
            const domSpan = currentDomElement.querySelector(".span");
            domSpan.classList.toggle(CLASSES.SPAN_COLOR);
            const tempFilterArray = [];
            domColorFilterButtons.forEach((button) => {
                var _a;
                if (button.classList.contains(CLASSES.FILTERS_COLOR_ACTIVE_BUTTON)) {
                    const dataFilter = button.getAttribute("data-filter");
    ...
    addEventListenersOnSizeFilters() {
        this.domSizeFilterButtons.forEach((domFilterButton) => {
            domFilterButton.addEventListener("click", (event) => {
                this.filters.filterSize = handleEvent(event, this.domSizeFilterButtons);
                this.ApplyParameters();
            });
        });
        function handleEvent(event, domSizeFilterButtons) {
            const currentDomElement = event.target;
            currentDomElement.classList.toggle(CLASSES.FILTERS_ACTIVE_BUTTON);
            const tempFilterArray = [];
    ...
    addEventListenersOnFavoriteFilters() {
        this.domFilterFavorite.addEventListener("click", () => {
            this.filters.filterFavorite = !this.domInputFavorite.checked;
            this.ApplyParameters();
        });
    }
    addEventListenersOnMusicButton() {
        this.buttonAudio.addEventListener("click", () => {
            ...
            this.buttonAudio.classList.toggle(CLASSES.SOUND_ON);
            this.manageMusic();
            this.localStorageFilters.setSettingsForSaving(this.filters, this.sortingMethod, this.isMusicOn);
        });
    }
    addEventListenerOnResetFilters() {
        ...
        domResetFilters.addEventListener("click", () => {
            domResetFilters.classList.add(CLASSES.RESET_PRESSED);
            setTimeout(() => {
                domResetFilters.classList.remove(CLASSES.RESET_PRESSED);
    ...
    clearActiveClasses() {
        this.domShapeFilterButtons.forEach((elem) => {
            const domCurrentFilter = elem;
            domCurrentFilter.classList.remove(CLASSES.FILTERS_ACTIVE_BUTTON);
        });
        this.domColorFilterButtons.forEach((elem) => {
            const domCurrentFilter = elem;
            domCurrentFilter.classList.remove(CLASSES.FILTERS_COLOR_ACTIVE_BUTTON);
            const domSpan = domCurrentFilter.querySelector(".span");
            domSpan.classList.remove("span-color");
        });
        this.domSizeFilterButtons.forEach((elem) => {
            const domCurrentFilter = elem;
            domCurrentFilter.classList.remove(CLASSES.FILTERS_ACTIVE_BUTTON);
        });
        this.domFilterFavorite.style.content = "";
        this.domInputFavorite.checked = false;
    ...
    manageMessageNoByFilters(param) {
        ...
        param === "show"
            ? messageNoByFilters.classList.remove(CLASSES.HIDDEN)
            : messageNoByFilters.classList.add(CLASSES.HIDDEN);
    }
    getDataFromLocalStorage() {
    ...
    showShapeFilters() {
        this.domShapeFilterButtons.forEach((element) => {
            var _a;
            ...
            if (this.filters.filterShape.includes(definitions_1.eShapes[instanceEShape])) {
                domButton.classList.toggle(CLASSES.FILTERS_ACTIVE_BUTTON);
            }
        });
    }
    showColorFilters() {
        this.domColorFilterButtons.forEach((element) => {
            var _a;
            const domButton = element;
            ...
            if (this.filters.filterColor.includes(definitions_1.eColors[instanceEColor])) {
                domButton.classList.toggle(CLASSES.FILTERS_COLOR_ACTIVE_BUTTON);
                domSpan.classList.toggle(CLASSES.SPAN_COLOR);
            }
        });
    }
    showSizeFilters() {
        this.domSizeFilterButtons.forEach((element) => {
            var _a;
            const domButton = element;
    ...
    showFavoriteFilter() {
        if (this.filters.filterFavorite) {
            this.domInputFavorite.checked = true;
        }
    }
    showSortingMethod() {
        var _a;
    ...
    showMusicSvg() {
        if (this.isMusicOn) {
            this.buttonAudio.classList.add(CLASSES.SOUND_ON);
        }
    }
    setRangeSliders() {
    ...
    manageMusic() {
        if (this.isMusicOn) {
            ...
            this.audio.pause();
            this.buttonAudio.classList.remove(CLASSES.SOUND_ON);
        }
    }
}
...
class Popup {
    ...
    showPopupWindow() {
        ...
        domBlurPopup.classList.remove(CLASSES.HIDDEN);
        setTimeout(() => {
            domPopup.classList.add(CLASSES.POPUP_OPACITY);
            setTimeout(() => {
                domPopup.classList.remove(CLASSES.POPUP_OPACITY);
                domBlurPopup.classList.add(CLASSES.HIDDEN);
            }, definitions_1.ANIMATION_TIME_500);
        }, definitions_1.ANIMATION_TIME_500);
    }
}
...
const CLASSES = {
    HIDDEN: "hidden",
    COLOR_BUTTON_ACTIVE: "color-button-active",
    ON_OFF_LIGHTS_OFF: "on-off-lights-off",
};
class Tree {
    constructor() {
        ...
        this.treePattern = 0;
        this.background = 0;
        this.lights = new lights_1.default("");
        this.localStorageTree = new LocalStorageTree_1.default();
        this.isLightsOn = false;
        this.isSnowOn = false;
        this.domTreeBackgroundImage = document.querySelector(".tree-container");
        this.domTreeImage = this.domTreeBackgroundImage.querySelector(".tree-pattern");
        this.domOrnamentsWrapper = document.querySelector(".ornaments-wrapper");
    }
    ...
    applyTreePattern() {
        this.domTreeImage.src = `${definitions_1.PATH_TO_TREES}${this.treePattern}.png`;
    }
    applyOrnaments() {
        const ornamentsArray = this.domTreeBackgroundImage.querySelectorAll(".toy-card-image");
    ...
    cloneOrnament(domElement, coordX, coordY) {
        ...
        domCloneOrnament.classList.remove(CLASSES.HIDDEN);
    }
    addOrnament(ornament, coordX, coordY) {
        this.domTreeBackgroundImage.append(ornament);
    ...
    changeTreeState(ornamentPattern, action) {
        const num = (0, generalFunctions_1.checkOnVoid)(ornamentPattern.getAttribute("data-num"), "0");
        ...
        if (domCard !== null) {
            ...
            if (action === "add") {
                ...
                if (currentNum === 1) {
                    domCardImg.classList.add(CLASSES.HIDDEN);
                    domCardCountWrapper.classList.add(CLASSES.HIDDEN);
                }
            }
            else if (action === "remove") {
                domCardCount.textContent = String(currentNum + 1);
                if (currentNum === 0) {
                    domCardImg.classList.remove(CLASSES.HIDDEN);
                    domCardCountWrapper.classList.remove(CLASSES.HIDDEN);
    ...
    findOrnamentByNum(num) {
        const cardArray = this.domOrnamentsWrapper.querySelectorAll(".decoration-card");
        let domCard = cardArray[0];
        for (let i = 0; i < cardArray.length; i++) {
            if (cardArray[i].hasAttribute("data-num") &&
                cardArray[i].getAttribute("data-num") === num) {
                domCard = cardArray[i];
                return domCard;
            }
        }
    ...
    gatherOrnamentsOnTree() {
        const ornamentsArray = this.domTreeBackgroundImage.querySelectorAll(".toy-card-image");
        const coordTree = this.domTreeBackgroundImage.getBoundingClientRect();
        this.ornaments = [];
        ornamentsArray.forEach((elem) => {
            const coord = elem.getBoundingClientRect();
            const num = (0, generalFunctions_1.checkOnVoid)(elem.getAttribute("data-num"), "0");
            this.ornaments.push({
                num: num,
    ...
    getDataFromLocalStorage() {
        ...
        this.lights =
            this.localStorageTree.lights != ""
                ? new lights_1.default(this.localStorageTree.lights)
                : new lights_1.default("");
        this.isLightsOn = this.localStorageTree.isLightsOn;
        this.isSnowOn = this.localStorageTree.isSnowOn;
        this.applyParameters();
    }
    addLights(color) {
        const domLightsWrapper = document.querySelector(".lights-wrapper");
        const buttonToggle = domLightsWrapper.querySelector(".on-off-lights");
        const buttonLight = domLightsWrapper.querySelector(`.${color}-button`);
        this.isLightsOn = true;
        buttonLight.classList.add(CLASSES.COLOR_BUTTON_ACTIVE);
        setTimeout(() => {
            buttonLight.classList.remove(CLASSES.COLOR_BUTTON_ACTIVE);
        }, definitions_1.ANIMATION_TIME_200);
        buttonToggle.textContent = "Выкл";
        buttonToggle.classList.remove(CLASSES.ON_OFF_LIGHTS_OFF);
        this.setLights(color);
        this.lights = new lights_1.default(color);
        this.localStorageTree.setSettingsForSaving(this);
    }
    setLights(color) {
        this.lights.createLights(color, this.isLightsOn);
    }
}
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.eSortingMethod = exports.eSizes = exports.eColors = exports.eShapes = exports.SNOWFLAKES_LIFESPAN = exports.LIGHTS_LINES_AMOUNT = exports.IS_SLIDER = exports.AMOUNT_OF_BACKGROUNDS = exports.AMOUNT_OF_TREES = exports.AMOUNT_OF_CARDS = exports.MUSIC_PATH = exports.YEAR_STEP = exports.YEAR_MAX = exports.YEAR_MIN = exports.AMOUNT_STEP = exports.AMOUNT_MAX = exports.AMOUNT_MIN = exports.CHOSEN_AMOUNT = exports.ANIMATION_TIME_200 = exports.ANIMATION_TIME_500 = exports.CHOSEN_TOYS = exports.PATH_TO_BACKGROUNDS = exports.PATH_TO_TREES = exports.PATH_TO_TOYS = void 0;
...
exports.LIGHTS_LINES_AMOUNT = 8;
exports.SNOWFLAKES_LIFESPAN = 6000;
var eShapes;
(function (eShapes) {
    eShapes["BALL"] = "\u0448\u0430\u0440";
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.dragAndDropsetEventListenerOnOrnaments = exports.activateDragAndDrop = void 0;
let toyMovedFromTree = false;
const domTreeContainer = document.querySelector(".tree-container");
const CLASSES = {
    MOVING: "moving",
};
function activateDragAndDrop(tree) {
    const map = domTreeContainer.querySelector(".treemap");
    domTreeContainer.ondrop = (event) => {
        drop(event, tree);
    };
    map.ondragover = (event) => {
        toyMovedFromTree = false;
        const ornament = document.querySelector(".moving");
        ornament.style.cursor = "pointer";
        allowDrop(event);
    };
    map.ondragleave = (event) => {
        toyMovedFromTree = true;
        dragleave(event);
    };
    const domArrayImages = domTreeContainer.querySelectorAll(".toy-card-image");
    domArrayImages.forEach((elem) => {
        elem.setAttribute("draggable", "true");
        elem.ondragstart = (event) => drag(event, false, tree);
    });
}
exports.activateDragAndDrop = activateDragAndDrop;
function allowDrop(event) {
    event.preventDefault();
}
function drag(event, isCloningNeed, tree) {
    if (event.dataTransfer != null) {
        event.dataTransfer.setData("text", String(isCloningNeed));
    }
    const ornament = event.target;
    ornament.classList.add(CLASSES.MOVING);
    ornament.ondragend = (event) => dragend(event, tree);
}
function dragleave(event) {
    event.preventDefault();
}
function dragend(event, tree) {
    if (toyMovedFromTree) {
        const ornament = document.querySelector(".moving");
        if (ornament) {
            ornament.remove();
            tree.changeTreeState(ornament, "remove");
        }
        tree.gatherOrnamentsOnTree();
        tree.localStorageTree.setSettingsForSaving(tree);
    }
}
function drop(event, tree) {
    const isCloningNeeded = event.dataTransfer.getData("text") === "true";
    event.preventDefault();
    const ornament = document.querySelector(".moving");
    const parent = ornament.parentElement;
    const ornamentWidth = ornament.width;
    const ornamentHeight = ornament.height;
    ornament.style.position = "absolute";
    ornament.style.width = `${ornamentWidth}px`;
    ornament.style.height = `${ornamentHeight}px`;
    ornament.style.zIndex = "5";
    const coordTree = domTreeContainer.getBoundingClientRect();
    const coord = ornament.getBoundingClientRect();
    domTreeContainer.appendChild(ornament);
    ornament.setAttribute("draggable", "true");
    ornament.ondragstart = (event) => drag(event, false, tree);
    ...
    ornament.classList.remove(CLASSES.MOVING);
    if (isCloningNeeded) {
        const cloneOrnament = ornament.cloneNode(true);
        cloneOrnament.style.position = "static";
        parent.append(cloneOrnament);
        cloneOrnament.ondragstart = (event) => drag(event, true, tree);
        tree.changeTreeState(cloneOrnament, "add");
    }
    tree.gatherOrnamentsOnTree();
    tree.localStorageTree.setSettingsForSaving(tree);
}
function dragAndDropsetEventListenerOnOrnaments(tree) {
    const domCards = document.querySelector(".ornaments-wrapper");
    const domArrayCards = domCards.querySelectorAll(".toy-card-image");
    domArrayCards.forEach((elem) => {
        elem.setAttribute("draggable", "true");
        elem.ondragstart = (event) => drag(event, true, tree);
    });
}
exports.dragAndDropsetEventListenerOnOrnaments = dragAndDropsetEventListenerOnOrnaments;
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.checkOnVoid = void 0;
function checkOnVoid(variable, bydefault) {
    return variable || bydefault;
}
exports.checkOnVoid = checkOnVoid;
...
class Lights {
    ...
    createLights(color, isLightsOn) {
        const lightsContainer = document.querySelector(".lights");
        lightsContainer.innerHTML = "";
        lightsContainer.classList.remove(CLASSES.HIDDEN);
        if (isLightsOn) {
            for (let i = 0; i < definitions_1.LIGHTS_LINES_AMOUNT; i++) {
                const lightsLevel = document.createElement("div");
                lightsLevel.classList.add(CLASSES.LIGHTS_LEVEL);
                for (let j = 0; j < i * 2 + 5; j++) {
                    const li = document.createElement("li");
                    li.classList.add(CLASSES.BULB, `bulb-${color}`);
...
function createYearRangeFilter() {
    const rangeSlider = document.getElementById("range-slider-year");
...
function selfestimation() {
  ...
  [x] 1. Вёрстка страниц приложения и навигация между ними +30
  [x] 2. Меню с настройками +50
  [x] 3. Гирлянда +40 
  [x] 4. Игрушки в избранном +80
  [x] 5. Дополнительный функционал на выбор 0
  Примечание.
  1.При выключении снег идет еще пару секунд, пока очищается setinterval
  2. Снег идет только на странице с елкой
  3. Елка сохраняется с гирляндой и игрушками
  `);
}
exports["default"] = selfestimation;
...
const definitions_1 = __webpack_require__(125);
const generalFunctions_1 = __webpack_require__(987);
const domCentralPart = document.querySelector(".central-part");
const CLASSES = {
    FA_SNOWFLAKE: "fa-snowflake",
};
function runSnowflake() {
    const coord = domCentralPart.getBoundingClientRect();
    const windowWidth = (0, generalFunctions_1.checkOnVoid)(Number(getComputedStyle(domCentralPart).width.slice(0, -2)), 500);
    const snowFlake = document.createElement("span");
    snowFlake.textContent = "❆";
    snowFlake.classList.add(CLASSES.FA_SNOWFLAKE);
    snowFlake.style.left = `${coord.x + Math.random() * windowWidth}px`;
    snowFlake.style.animationDuration = Math.random() * 3 + 2 + "s";
    snowFlake.style.opacity = String(Math.random());
    snowFlake.style.fontSize = Math.random() * 10 + 10 + "px";
    domCentralPart.append(snowFlake);
    console.log(snowFlake.style.left);
    setTimeout(() => {
        snowFlake.remove();
    }, definitions_1.SNOWFLAKES_LIFESPAN);
}
